...
def main() -> int:
    ...
    try:
        ...
        # indefinitely, and PEP 475 retries the syscall after a handler that
        # only sets an Event — so SIGTERM left the loop waiting for the NEXT
        # request and systemd stop degraded to SIGKILL, losing the
        # clean-shutdown exit-0 contract. A 1s server timeout makes the loop
        # re-check `stop` promptly.
        server.timeout = 1.0
