...
def test_check_spec_coherence_alerts_on_failed_run(tmp_path):
    ...
    server, port = _start_fake_gh({
        ...
        "/repos/owner/repo/actions/workflows/42/runs?per_page=1": lambda: (200, {
            "workflow_runs": [
                {"conclusion": "failure", "completed_at": _iso(
...
def test_check_spec_coherence_healthy_run_does_not_alert(tmp_path):
    ...
    server, port = _start_fake_gh({
        ...
        "/repos/owner/repo/actions/workflows/42/runs?per_page=1": lambda: (200, {
            "workflow_runs": [
                {"conclusion": "success", "completed_at": _iso(
                    datetime.now(timezone.utc))}
            ]}),
    })
    try:
...
def test_poll_repo_runs_the_coherence_check(tmp_path):
    ...
    server, port = _start_fake_gh({
        ...
        "/repos/owner/repo/actions/workflows/42/runs?per_page=1": lambda: (200, {
            "workflow_runs": [
                {"conclusion": "failure", "completed_at": _iso(now),
                 "html_url": "https://example.invalid/run/1"}
            ]}),
    })
    try:
