...
The framework runs its own medicine: CI executes the full test suite and the
shipped gates against DevGate's own tree on every pull request and push to
main, with deterministic test collection and immutably pinned actions. A repo
whose product is enforcement gates cannot merge changes its own gates would
reject.
