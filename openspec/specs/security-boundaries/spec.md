...
Trust boundaries hold even when the input is hostile: repository-supplied
paths are contained before any read, the monitor hub bounds request bodies,
shuts down on signal, and decides enrollment atomically, token material is
never stored or logged in plaintext, and every shipped workflow action and
container image is immutably referenced.
