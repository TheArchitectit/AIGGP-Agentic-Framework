...
Scaling a DevGate-managed runner fleet past its first machine: the runner
templates include a documented procedure for adding the N+1th runner — shared
entrypoint via bind-mount, per-runner token drop-in, distinct names and labels
— without forking the shipped quadlet or leaking host-specific detail into the
public templates.
