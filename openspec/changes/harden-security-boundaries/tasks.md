...
- [ ] 1.1 `package.resolve`: validate each inventory entry's `path` against
       root containment (reuse `manifest._check_safe` semantics); traversal
       attempts are PackageError (exit 30), never reads.
- [ ] 1.2 `evaluators._extract`: normalize + contain file-selector paths under
       subject_root; reject `..`/absolute/symlink-escape as Unresolved with a
       stable reason.
- [ ] 1.3 `manifest.build`: single read — hash and size from the same bytes
       (stream: update digest and counter from each chunk).
- [ ] 1.4 Behavioral tests: hostile inventory path rejected without reading;
...
- [ ] 2.1 `hub/main.py`: signal-safe serve loop (`serve_forever()` +
...
- [ ] 2.4 `registry`: store `heartbeat_token` as
       ...
       the stored verifier.
- [ ] 2.5 `monitor._check_queue_drain`: `run.get("id")` (fix dedupe key);
       include `html_url`/`name` in the detail.
- [ ] 2.6 `config.watched_branches` default: resolve the repo default branch
...
- [ ] 3.2 `runner-enroll.sh`: build JSON via `python3 - <<PY` heredoc (json.dumps)
       ...
       ExecStart.
- [ ] 3.3 One canonical secrets mechanism across all three runner docs +
...
- [ ] 3.4 Pin template images: `actions-runner` by digest;
...
- [ ] 3.5 `detect-host-ci.py`: word-boundary the redaction alternation (drop
       bare `ak|sk`), handle rglob errors, support `*.yaml` workflows.
- [ ] 3.6 Tests: enroll payload escaping (label with quote/backslash);
       hub response with token never appears on stdout (assert on captured
       output).

## 4. Registry and advisory

- [ ] 4.1 Failure-registry entries for the containment and token-echo classes.
- [ ] 4.2 docs/runner-monitor-monitor-hub.md: document the hashed-at-rest
