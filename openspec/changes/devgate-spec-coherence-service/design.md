...
Confirmed against fleet reconnaissance on 2026-09-17, **re-confirmed 2026-09-25** against live state after the two-tier rebalance (sources: framework repo + private `TheArchitectit/infra-info`):
...
- **Runner hosts are split by tier since 2026-09-25** (owner call): **ucs03** is the CPU tier — 15 repo-scoped runners, all non-GPU CI — and **dell-u2 is GPU-only**, running exactly two runners (`*-gpu` labels, CDI-pinned) plus the hub. The split rule matters for this package: **no runner may carry both a bare and a `-gpu` label**, or CPU jobs siphon back onto the GPU box. The framework's own runner is `ucs03-devgate` (ucs03). `100.107.9.85` is paused for a hardware swap.
- **gamerepo01 is a real registered fleet repo** — its live runner is `ucs03-game` (labels `devgate-game,devgate`; the `dell-u2-game` GPU runner that hosted it on 2026-09-17 is now archived offline, and `u85-game` is paused with the .85 swap). So the coherence pilot's motivating subject exists in the fleet, and its lane moved hosts in the same rebalance. The lineage-mismatch narrative remains uncaptured (R9 provenance still applies: synthetic fixture until captured from the real repo with owner approval).
- **⚠️ Coverage caveat for the hub check class below**: only 4 of ucs03's 15 lanes carry heartbeat spokes, and `/health`'s `registered_runners` counts registry entries rather than monitored runners (13 entries, 7 enrolled as of 2026-09-25). A new check class inherits that gap — adding `_check_spec_coherence` to the poll loop does not widen which runners are polled.
