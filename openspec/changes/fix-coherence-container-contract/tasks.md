...
- [ ] 1.1 `git mv openspec/changes/devgate-spec-coherence-service/schemas
       hub/coherence/schemas` (13 files); leave a pointer note in the change
       package so its docs stay navigable.
- [ ] 1.2 `hub/coherence/__main__.py`: `SCHEMA_DIR = Path(__file__).parent /
       "schemas"`; delete the change-package path and its comment.
- [ ] 1.3 `hub/coherence/schemacheck.py`: same resolution.
- [ ] 1.4 Grep the tree for the old path string; update
       `tests/test_hub_coherence_schema.py` and any docs/design references.
- [ ] 1.5 Host-side regression: full `pytest tests/test_hub_coherence*.py`
...
- [ ] 3.1 Add `TestContainerExecReal::test_incontainer_valid_request_passes`:
...
- [ ] 3.2 Keep the exit-30 rejection case but tighten it: assert the envelope
...
- [ ] 4.1 Append an S4 regression entry to
...
- [ ] 4.3 Failure-registry entry for the incident class
       ("smoke test asserts an exit code achievable by a broken service").

## 5. Spec delta

- [ ] 5.1 `specs/container-runtime/spec.md` delta (see specs/ in this change):
