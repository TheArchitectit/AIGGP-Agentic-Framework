...
- [ ] 1.1 hub/ Python service: /enroll, /heartbeat, /health endpoints +
      runners.json load/save on the hub volume (D1, D3)
...
- [ ] 4.1 Alert engine: dedupe by (repo, check-class, runner), JSONL alert
      log on hub volume
- [ ] 4.2 GitHub-issue notifier with `devgate-monitor` label + notifier
      interface stub
- [ ] 4.3 Dead-man switch workflow template for this repo (risk table)

## Sprint 5 — monitor-hub deployment runbook (D7)
- [ ] 5.1 docs/runner-monitor-monitor-hub.md: volume, token drop-in, enrollment
      tokens, port/firewall, linger, rotation (D7)
